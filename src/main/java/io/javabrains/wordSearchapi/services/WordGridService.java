       ...
       char[][] contents=new char[gridSize][gridSize];
        ...
        for(String word:words){
            ...
            for(Coordinate cord:coordinates){
                int x=cord.x;
                int y= cord.y;
                Direction selectedDirection = getDirectionForFit(contents,word,cord);
    ...
    private boolean doesFit(char[][] contents,String word,Coordinate coordinate,Direction direction){
        ...
        switch(direction){
            ...
            case VERTICAL:
                ...
                break;
            case DIAGONAL:
                if(coordinate.x+wordLength>gridSize || coordinate.y+word.length()>gridSize) return false;
                for(int i=0;i<wordLength;i++) {
            ...
            case VERTICAL_INVERSE:
                ...
                break;
            case DIAGONAL_INVERSE:
                if(coordinate.x<wordLength || coordinate.y<word.length()) return false;
                for(int i=0;i<wordLength;i++) {
                    if (contents[coordinate.x-i][coordinate.y - i] != '_') return false;
                }
                break;
